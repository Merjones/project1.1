    ...
    for m_ in m:
        for num in numMergedFeatures:
            ...
            for i in range(num):
                ...
                selectedMergedFeatures[i] = col

            sc = StandardScaler()
            sc.fit(selectedMergedFeatures)
            features = sc.transform(selectedMergedFeatures)

            clf = svm.SVC(kernel='linear', C=1)
            #
            mergedScores = cross_val_score(clf, features, massType, cv=10)
            print("merged average using:", m, "training instances is:", mergedScores.mean())
            ...
            result = [m_, num, mergedScores.mean(),averageAccuracy]
            print(result)
